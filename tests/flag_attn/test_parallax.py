    ...
    for batch_index in range(B):
        ...
        for head_index in range(H):
            ...
            for group_index in range(G):
                ...
                output = mean_value * (1.0 + correction.sum(dim=-1, keepdim=True)) - correction @ value
...
def _absolute_error(expected: torch.Tensor, actual: torch.Tensor) -> float:
    return (expected.detach().float() - actual.detach().float()).abs().max().item()


def _error_ratio(expected: torch.Tensor, actual: torch.Tensor) -> float:
    error = (expected.detach().float() - actual.detach().float()).square().mean().sqrt()
...
def _assert_close(
    ...
    *,
    ...
    assert expected.shape == actual.shape, f"{name}: shape mismatch: expected {expected.shape}, got {actual.shape}"
    assert expected.dtype == actual.dtype, f"{name}: dtype mismatch: expected {expected.dtype}, got {actual.dtype}"
    assert torch.isfinite(expected).all(), f"{name}: reference contains NaN or Inf"
    assert torch.isfinite(actual).all(), f"{name}: result contains NaN or Inf"

    absolute_error = _absolute_error(expected, actual)
    ratio = _error_ratio(expected, actual)
    message = f"{name:>16} diff: {absolute_error:.6f} ratio: {ratio:.6f}"
    logger.info(message)
    ...
    allow_ci_warning = FLA_CI_ENV and (ratio < 0.01 or absolute_error <= 0.3)
    if warning or allow_ci_warning:
        if ratio >= tolerance:
            warnings.warn(message, stacklevel=2)
        return absolute_error, ratio

    assert ratio < tolerance, f"{message}; tolerance: {tolerance:.6f}"
    return absolute_error, ratio
...
def _print_error_report(
    ...
    title = f"parallel_parallax | {case.name} | dtype={dtype_name} | tolerance={tolerance:.6f}"
    separator = "-" * len(title)
    rows = [
        "",
        separator,
    ...
    for name, absolute_error, ratio in metrics:
        status = "PASS" if absolute_error <= 1e-6 or ratio < tolerance else "WARN"
        rows.append(f"{name:<8} {absolute_error:>16.6e} {ratio:>16.6e} {status:>10}")
