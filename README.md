...
The scheduler runs accuracy tests before benchmarks. Each operator's human-readable output is saved as `performance_stdout.log`; structured records for each benchmark script are saved separately as `performance_records_0.log`, `performance_records_1.log`, and so on, under the operator's output directory. These records use FlagGems-compatible `[INFO] {` JSON lines containing shapes, baseline latency, FlagAttention latency, and speedup when a baseline is available. Raw records use `speedup: null` without a baseline; `--record json` omits those shape rows so FlagGems reports the dtype as `Unknown`. Direct script execution without a record path still prints these records to stdout for older schedulers; set `FLAG_ATTN_BENCHMARK_LOG_PATH=/path/to/records.log` to keep direct console output free of JSON. Existing tables, plots, and matmul-based throughput remain available; the `flash_decoding_benchmark.py` plot axis now correctly says milliseconds. Historical v0.2 plots remain under [`assets/v0.2`](./assets/v0.2); rerun the current benchmark for conclusions about current code, Triton, and hardware.
