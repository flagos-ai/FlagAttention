...
K1_TLE_MAXNREG_CANDIDATES = (64, 72, 96)
...
if HAS_TLE_GDN2:
    ...
    @triton.heuristics(
        {
            ...
            "USE_GATE_IN_KERNEL": lambda args: args["A_log"] is not None,
                ...
                "USE_K1_SMEM_REUSE": lambda args: args["K"] >= K1_TLE_SMEM_REUSE_MIN_K,
        }
    )
    @triton.autotune(
        configs=[
            ...
            for maxnreg in [None, *K1_TLE_MAXNREG_CANDIDATES]
        ],
        key=["H", "K", "V", "BT"],
    ...
    def _chunk_gdn2_fwd_intra_infer_kernel(
        ...
        USE_GATE_IN_KERNEL: tl.constexpr,
          ...
          USE_K1_SMEM_REUSE: tl.constexpr,
      ):
        i_t = tl.program_id(0)
        ...
        if USE_K1_SMEM_REUSE:
            # q/k/g/b are consumed by multiple phases.  Keep one CTA-local copy
            # so maxnreg can shorten their register live ranges without reloading
            ...
            full_rows = tl.broadcast_to(tl.arange(0, BT)[:, None], (BT, K))
            full_cols = tl.broadcast_to(tl.arange(0, K)[None, :], (BT, K))
            q_sp = tle.gpu.local_ptr(q_buf, (full_rows, full_cols))
            k_sp = tle.gpu.local_ptr(k_buf, (full_rows, full_cols))
            g_sp = tle.gpu.local_ptr(g_buf, (full_rows, full_cols))
            b_sp = tle.gpu.local_ptr(b_buf, (full_rows, full_cols))

            p_q = tl.make_block_ptr(q, (T, K), (H * K, 1), (i_t * BT, 0), (BT, K), (1, 0))
            p_k = tl.make_block_ptr(k, (T, K), (H * K, 1), (i_t * BT, 0), (BT, K), (1, 0))
            p_g = tl.make_block_ptr(g, (T, K), (H * K, 1), (i_t * BT, 0), (BT, K), (1, 0))
            p_b = tl.make_block_ptr(b, (T, K), (H * K, 1), (i_t * BT, 0), (BT, K), (1, 0))
            tl.store(q_sp, tle.load(p_q, boundary_check=(0, 1), is_async=True))
            tl.store(k_sp, tle.load(p_k, boundary_check=(0, 1), is_async=True))
            tl.store(g_sp, tle.load(p_g, boundary_check=(0, 1), is_async=True).to(tl.float32))
            tl.store(b_sp, tle.load(p_b, boundary_check=(0, 1), is_async=True))
        ...
        if USE_QK_L2NORM:
            ...
            for i_k in range(tl.cdiv(K, BK)):
                if USE_K1_SMEM_REUSE:
                    tile_rows = tl.broadcast_to(tl.arange(0, BT)[:, None], (BT, BK))
                    tile_cols = tl.broadcast_to(
                        (i_k * BK + tl.arange(0, BK))[None, :], (BT, BK)
                    )
                    m_k = tile_cols < K
                    b_q_blk = tl.load(
                        tle.gpu.local_ptr(q_buf, (tile_rows, tile_cols)),
                        mask=m_k,
                        other=0.0,
                    ).to(tl.float32)
                    b_k_blk = tl.load(
                        tle.gpu.local_ptr(k_buf, (tile_rows, tile_cols)),
                        mask=m_k,
                        other=0.0,
                    ).to(tl.float32)
                else:
        ...
        for i_k in range(tl.cdiv(K, BK)):
            if USE_K1_SMEM_REUSE:
                ...
                m_k = tile_cols < K
                q_tile = tle.gpu.local_ptr(q_buf, (tile_rows, tile_cols))
                k_tile = tle.gpu.local_ptr(k_buf, (tile_rows, tile_cols))
                g_tile = tle.gpu.local_ptr(g_buf, (tile_rows, tile_cols))
                b_tile = tle.gpu.local_ptr(b_buf, (tile_rows, tile_cols))
                b_q_blk = tl.load(q_tile, mask=m_k, other=0.0).to(tl.float32) * b_q_rstd[:, None]
                b_k_blk = tl.load(k_tile, mask=m_k, other=0.0).to(tl.float32) * b_k_rstd[:, None]
                b_g_blk = tl.load(g_tile, mask=m_k, other=0.0)
                b_erase_blk = tl.load(b_tile, mask=m_k, other=0.0).to(tl.float32)
            else:
                p_q = tl.make_block_ptr(
                    q, (T, K), (H * K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0)
                )
            ...
            if USE_K1_SMEM_REUSE:
                tl.store(g_tile, b_g_blk, mask=m_k)
        ...
        for i_k in range(tl.cdiv(K, BK)):
            if USE_K1_SMEM_REUSE:
                ...
                b_k_blk = tl.load(k_tile, mask=m_k, other=0.0).to(tl.float32) * b_k_rstd[:, None]
                b_erase_blk = tl.load(b_tile, mask=m_k, other=0.0).to(tl.float32)
                b_gk_blk = tl.load(g_tile, mask=m_k, other=0.0)
            else:
                p_k = tl.make_block_ptr(
                    k, (T, K), (H * K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0)
                )
            ...
            if STORE_KG:
                if USE_K1_SMEM_REUSE:
                    ...
                    last_rows = tl.full([BK], last_local, dtype=tl.int32)
                    last_cols = i_k * BK + tl.arange(0, BK)
                    b_gn = tl.load(
                        tle.gpu.local_ptr(g_buf, (last_rows, last_cols)),
                        mask=last_cols < K,
                        other=0.0,
                    )
                else:
