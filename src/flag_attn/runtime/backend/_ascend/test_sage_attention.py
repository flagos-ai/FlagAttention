...
def test_forward_matches_dequantized_reference(tensor_layout, num_kv_heads):
    ...
    q = torch.randn(batch_size, num_query_heads, seq_len, head_dim, device="npu", dtype=torch.float16)
    k = torch.randn(batch_size, num_kv_heads, seq_len, head_dim, device="npu", dtype=torch.float16)
    v = torch.randn(batch_size, num_kv_heads, seq_len, head_dim, device="npu", dtype=torch.float16)
...
def test_forward_rejects_non_positive_maxnreg():
    q = torch.zeros((1, 1, 128, 64), device="npu", dtype=torch.int8)
    k = torch.zeros_like(q)
    v = torch.zeros(q.shape, device="npu", dtype=torch.float16)
    q_scale = torch.ones((1, 1, 1), device="npu")
    k_scale = torch.ones((1, 1, 2), device="npu")
...
def test_forward_supports_masks_and_partial_blocks(mask_kind):
    torch.manual_seed(7)
    q = torch.randn((1, 1, 129, 128), device="npu", dtype=torch.float16)
    k = torch.randn((1, 1, 70, 128), device="npu", dtype=torch.float16)
    v = torch.randn_like(k)

    if mask_kind == "bool":
        attn_mask = torch.ones((1, 1, 129, 70), device="npu", dtype=torch.bool)
        attn_mask[..., ::3] = False
    else:
        attn_mask = torch.zeros((1, 1, 129, 70), device="npu", dtype=torch.float32)
        attn_mask[..., ::3] = -2.0
