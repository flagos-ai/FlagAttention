...
@triton.autotune(
    configs=[
        triton.Config({"BK": 16, "BH": 32}, num_warps=2, num_stages=1),
    ],
    key=[
        "SQ",
        "HQ",
...
hygon_flash_mla_sparse_tle_fwd = triton_flash_mla_sparse_fwd
...
def flash_mla_sparse_fwd(
    ...
    can_use_tle = _can_use_tle_flash_mla_sparse_fwd(
        q, kv, indices, d_v, topk_length
    )
    if can_use_tle and vendor_name == "hygon":
        hygon_flash_mla_sparse_tle_fwd[triton_grid](
            q,
            kv,
            indices,
            attn_sink,
