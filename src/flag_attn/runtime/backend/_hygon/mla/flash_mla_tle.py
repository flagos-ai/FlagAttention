...
from flag_attn.runtime.backend._hygon import device, error, torch_device_fn  # noqa: E402
from flag_attn.runtime.backend._hygon.device_info import get_device_capability  # noqa: E402
from flag_attn.gated_delta_rule.compat import has_triton_tle  # noqa: E402
...
FLASH_MLA_HYGON_BLOCK_H = 16
FLASH_MLA_HYGON_BLOCK_N = 32
FLASH_MLA_HYGON_BLOCK_D = 256
...
@triton.jit
def flash_mla_splitkv_hygon_kernel(
    Q,
    Kv,
    Block_table,
    B_seq_len,
    ...
    meta = Sched_meta + partition_idx * META_FIELDS
    begin_req_idx = tl.load(meta + 0)
    end_req_idx = tl.load(meta + 1)
    begin_block_idx_meta = tl.load(meta + 2)
    end_block_idx_meta = tl.load(meta + 3)
    begin_split_idx = tl.load(meta + 4)
    is_first_req_splitted = tl.load(meta + 5) != 0
    is_last_req_splitted = tl.load(meta + 6) != 0

    offs_h = head_block_idx * BLOCK_H + tl.arange(0, BLOCK_H)
    offs_n = tl.arange(0, BLOCK_N)
    offs_v = value_block_idx * BLOCK_D + tl.arange(0, BLOCK_D)
    mask_h = offs_h < head_num
    mask_v = offs_v < HEAD_DIM_V

    offs_d0 = tl.arange(0, 256)
    offs_d1 = 256 + tl.arange(0, 256)
    offs_dt = HEAD_DIM_V + tl.arange(0, HEAD_DIM - HEAD_DIM_V)
    ...
    for batch_idx in tl.range(begin_req_idx, end_req_idx + 1):
        ...
        split_offset = tl.where(batch_idx == begin_req_idx, begin_split_idx, 0)
        split_idx = tl.load(Num_splits + batch_idx) + split_offset
        ...
        q_base = Q + batch_idx * stride_q_b + offs_h[:, None] * stride_q_h
        q0 = tl.load(q_base + offs_d0[None, :], mask=mask_h[:, None], other=0.0)
        q1 = tl.load(q_base + offs_d1[None, :], mask=mask_h[:, None], other=0.0)
        qt = tl.load(q_base + offs_dt[None, :], mask=mask_h[:, None], other=0.0)

        max_log = tl.full([BLOCK_H], float("-inf"), tl.float32)
        ...
        for block_idx in tl.range(start_block_idx, end_block_idx):
            token_offsets = block_idx * BLOCK_N + offs_n
            valid_n = token_offsets < seq_len

            token_base = block_idx * BLOCK_N
            page_slot = token_base // PAGE_SIZE
            offset_in_page = token_base % PAGE_SIZE + offs_n
            page = tl.load(
                Block_table + batch_idx * stride_block_table_b + page_slot
            )
            kv_rows = (
                page.to(tl.int64) * PAGE_SIZE
                + offset_in_page.to(tl.int64)
            )
            kv_base = Kv + kv_rows[None, :] * stride_kv_token

            k0 = tl.load(
                kv_base + offs_d0[:, None],
                mask=valid_n[None, :],
                other=0.0,
            )
            k1 = tl.load(
                kv_base + offs_d1[:, None],
                mask=valid_n[None, :],
                other=0.0,
            )
            kt = tl.load(
                kv_base + offs_dt[:, None],
                mask=valid_n[None, :],
                other=0.0,
            )

            qk = tl.dot(q0, k0, out_dtype=tl.float32)
            qk = tl.dot(q1, k1, qk, out_dtype=tl.float32)
            qk = tl.dot(qt, kt, qk, out_dtype=tl.float32)
            qk *= sm_scale
            qk = tl.where(valid_n[None, :], qk, float("-inf"))

            new_max = tl.maximum(max_log, tl.max(qk, axis=1))
            alpha = tl.exp(max_log - new_max)
            probs = tl.exp(qk - new_max[:, None])

            values = tl.load(
                Kv
                + kv_rows[:, None] * stride_kv_token
                + offs_v[None, :],
                mask=valid_n[:, None] & mask_v[None, :],
                other=0.0,
            )

            acc *= alpha[:, None]
            acc = tl.dot(
                probs.to(values.dtype),
                values,
                acc,
        ...
        if is_no_split:
            tl.store(
                ...
                + offs_v[None, :],
                normalized.to(O.dtype.element_ty),
            ...
            tl.store(
                ...
                + offs_v[None, :],
                normalized.to(O_accum.dtype.element_ty),
                mask=mask_h[:, None] & mask_v[None, :],
            )
...
class FlashMLATLEDecodePlan:
    def __init__(
        ...
        *,
        ...
        self.use_hygon_compat = vendor_name == "hygon"
        plan_block_h = FLASH_MLA_HYGON_BLOCK_H if self.use_hygon_compat else FLASH_MLA_BLOCK_M
        self.num_m_blocks = triton.cdiv(s_q * h_q // h_kv, plan_block_h)
    ...
    def plan(self, cache_seqlens: torch.Tensor) -> None:
        ...
        flash_mla_sched_meta_kernel_v3[(1,)](
            ...
            BATCH_SIZE=self.b,
            BLOCK_SIZE_N=(
                FLASH_MLA_HYGON_BLOCK_N
                if self.use_hygon_compat
                else FLASH_MLA_BLOCK_N
            ),
    ...
    def run(
        ...
        *,
        ...
        if self.use_hygon_compat:
            max_cache_capacity = (
                block_table_tle.shape[1] * self.block_size
            )
            hygon_num_warps = (
                4 if max_cache_capacity >= 12000 else 2
            )

            flash_mla_splitkv_hygon_kernel[
                (
                    self.num_m_blocks,
                    self.num_sm_parts,
                    triton.cdiv(self.dv, FLASH_MLA_HYGON_BLOCK_D),
                )
            ](
                q_tle.view(self.b * self.s_q, self.h_q, self.d),
                kv_flat,
                block_table_tle,
                ...
                BLOCK_H=FLASH_MLA_HYGON_BLOCK_H,
                BLOCK_N=FLASH_MLA_HYGON_BLOCK_N,
                BLOCK_D=FLASH_MLA_HYGON_BLOCK_D,
                PAGE_SIZE=self.block_size,
                HEAD_DIM_V=self.dv,
                HEAD_DIM=self.d,
                META_FIELDS=FLASH_MLA_META_FIELDS,
                num_warps=hygon_num_warps,
                num_stages=1,
            )
            q_desc = q_tail_desc = output_desc = oaccum_desc = None
            kv_desc = kv_tail_desc = None
        else:
            TensorDescriptor = _get_tensor_descriptor_cls()
            _ensure_triton_descriptor_allocator(self.device)
...
def flash_mla_attn_kernel(
    ...
    cur_head_id = tl.program_id(0).to(tl.int64)
    cur_batch_id = tl.program_id(1).to(tl.int64)
