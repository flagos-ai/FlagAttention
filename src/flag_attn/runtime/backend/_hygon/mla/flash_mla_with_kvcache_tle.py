...
@triton.autotune(
    configs=[
        triton.Config({"BK": 16, "BH": 64}, num_warps=4, num_stages=1),
    ],
...
def _dense_decode_dispatch(
    ...
    if _can_use_tle_dense_decode(
        q, kv_cache, block_table, head_dim_v, page_block_size
    ) and not _use_hygon_tle_compat():
...
def _use_hygon_tle_compat() -> bool:
    return vendor_name == "hygon" and HAS_TLE and _tle_decode_enabled()
