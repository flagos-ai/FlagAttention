...
from flag_attn.FLA.parallax.parallel import (
...
class BenchConfig:
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            raise ValueError("PARALLAX_BENCH_MODE must be kernel, operator, or wrapper") from exc
        ...
        if dtype_names:
            ...
            if unknown:
                raise ValueError(f"PARALLAX_BENCH_DTYPES only supports float16,bfloat16; got {','.join(unknown)}")
...
def _build_inputs(
    ...
    if case.cu_seqlens is not None:
        if case.B != 1 or case.cu_seqlens[0] != 0 or case.cu_seqlens[-1] != case.T:
            raise ValueError("A variable-length case requires B=1 and cu_seqlens spanning [0, T]")
    ...
    window_size_left = -1 if case.window_size is None else case.window_size
...
def _warm_up_pair(
    ...
    if Config.mode.value == "kernel":
        ...
        for cycle in range(Config.warm_up):
            functions = (fla_fn, flag_attn_fn) if cycle % 2 == 0 else (flag_attn_fn, fla_fn)
            for fn in functions:
                fn()
...
def _median_and_mad_pct(
    ...
    absolute_deviations = [abs(sample - center) for sample in samples]
...
def _bench_balanced_pair(
        ...
        for cycle in range(BALANCED_MEASUREMENT_CYCLES):
            for fn in _measurement_order(
                ...
                if not math.isfinite(sample_ms) or sample_ms <= 0.0:
                    raise RuntimeError(f"invalid latency sample: {sample_ms}")
    ...
    fla_ms, fla_mad_pct = _median_and_mad_pct(fla_samples)
    flag_attn_ms, flag_attn_mad_pct = _median_and_mad_pct(flag_attn_samples)
...
def _bench_single(flag_attn_fn: Callable[[], object]) -> PhaseBenchmarkResult:
    ...
    flag_attn_ms, flag_attn_mad_pct = _median_and_mad_pct(samples)
    return PhaseBenchmarkResult(
        fla_ms=None,
        flag_attn_ms=flag_attn_ms,
        fla_mad_pct=None,
...
def _relative_rms(
    ...
    if reference.shape != actual.shape:
        raise AssertionError(f"{name}: shape mismatch: {reference.shape} != {actual.shape}")
...
def _validate_pair(
    ...
    if max_ratio >= tolerance:
        details = ", ".join(f"{name}={ratio:.3e}" for name, ratio in zip(names, ratios))
        raise AssertionError(
            f"{phase} sampled relative RMS error {max_ratio:.3e} exceeded tolerance {tolerance:.3e}: {details}"
        )
    return max_ratio
...
def _benchmark_case(
    ...
    if FLA_IMPORT_ERROR is not None:
        return _bench_single(flag_attn_fn)
...
def _selected_dtypes() -> list[torch.dtype]:
    dtypes = Config.user_desired_dtypes or DEFAULT_DTYPES

    unsupported = [dtype for dtype in dtypes if dtype not in SUPPORTED_DTYPES]

    if unsupported:
        names = ", ".join(str(dtype) for dtype in unsupported)
        raise ValueError(f"parallel_parallax does not support: {names}")
...
def _print_result(
    ...
    latency_values = [result.flag_attn_ms]
    if result.fla_mad_pct is not None:
        mad_values.append(result.fla_mad_pct)
    if result.fla_ms is not None:
        latency_values.append(result.fla_ms)

    max_mad = max(mad_values)
    is_short_latency = max(latency_values) < SHORT_LATENCY_THRESHOLD_MS
    warning_threshold = SHORT_LATENCY_MAD_WARNING_PCT if is_short_latency else MAD_WARNING_THRESHOLD_PCT
    if max_mad > warning_threshold:
        print(f"[WARN unstable] maximum MAD={max_mad:.3f}% exceeds {warning_threshold:.1f}%")
...
def test_perf_parallel_parallax() -> None:
    ...
    if FLA_IMPORT_ERROR is None:
        print(f"measurement_order = mirrored ABBA/BAAB; samples/provider = {BALANCED_MEASUREMENT_CYCLES * 2}")
    else:
        print(f"samples/provider = {BALANCED_MEASUREMENT_CYCLES * 2}")
    if Config.mode.value == "kernel":
        print("timing = Triton per-invocation device timing; warmup/iter are per-sample millisecond budgets")
    else:
